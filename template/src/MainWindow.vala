namespace GraniteWallpaperTestApp {
    public class MainWindow : Hdy.ApplicationWindow {
        public weak GraniteWallpaperTestApp.Application app { get; construct; }

        // Widgets
        public GraniteWallpaperTestApp.Widgets.HeaderBar toolbar;
        ...
        public MainWindow (GraniteWallpaperTestApp.Application application) {
            Object (
                ...
                icon_name: "com.github.Antolius.granite_welcome_test"
            );
        }
        ...
        private void create_layout () {
            ...
            toolbar = new GraniteWallpaperTestApp.Widgets.HeaderBar ();
        ...
        private void restore_window_state () {
            ...
            GraniteWallpaperTestApp.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = GraniteWallpaperTestApp.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    GraniteWallpaperTestApp.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                GraniteWallpaperTestApp.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                GraniteWallpaperTestApp.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                GraniteWallpaperTestApp.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                GraniteWallpaperTestApp.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            GraniteWallpaperTestApp.saved_state.set ("window-position", "(ii)", x, y);
        }
